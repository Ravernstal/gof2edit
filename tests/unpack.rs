...
#[test]
fn convert_agents_bin_to_json() {
    let agents =
        serde_json::from_str::<Vec<Agent>>(AGENTS_JSON).expect("failed to parse agent test JSON");

    let mut buffer = Cursor::new(vec![0u8]);

    gof2edit::write_object_list::<Agent, BigEndian>(&mut buffer, agents)
        .expect("failed to repack agents to BIN");
    buffer.set_position(0);

    let agents = gof2edit::read_object_list::<Agent, BigEndian>(&mut buffer)
        .expect("failed to unpack agents from BIN");

    assert!(agents.is_empty().not())
}
