...
impl BinRead for Option<SaveAgentMission> {
    fn read_bin<O: ByteOrder>(source: &mut impl Read) -> Result<Self> {
        let has_mission = source.read_i32::<O>()?;

        if has_mission == -1 {
            return Ok(None);
        }

        Ok(Some(SaveAgentMission {
            mission_type: source.read_i32::<O>()?,
