...
impl BinRead for Option<UnknownStructure1> {
    fn read_bin<O: ByteOrder>(source: &mut impl Read) -> io::Result<Self> {
        ...
        let unknown_string_list_1 = (0..count).map(|_| WideString::read_bin::<O>(source)).collect::<io::Result<Vec<_>>>()?;
        let unknown_string_list_1 = unknown_string_list_1.into_iter().map(|string| string.get()).collect();
